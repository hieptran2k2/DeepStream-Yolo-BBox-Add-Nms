...
public:
    ...
    std::vector<color_tp> pallete() {
        ...
        std::vector<color_tp> colors;
        for (auto& [vbox, avg_color] : vboxes.get_contents()) {
            uint8_t percent = (uint8_t)(((float) vbox.count()) / (float) total * 100);
            color_tp x;

            std::get<0>(x) = std::get<0>(avg_color);
            std::get<1>(x) = std::get<1>(avg_color);
            std::get<2>(x) = std::get<2>(avg_color);
            std::get<3>(x) = percent;
            colors.push_back(x);
        }
        return colors;
