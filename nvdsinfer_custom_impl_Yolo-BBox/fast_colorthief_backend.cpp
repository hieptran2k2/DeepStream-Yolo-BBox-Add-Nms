...
std::vector<color_tp> get_palette_mask_c(uint8_t* data, uint8_t* data_mask, int color_count, int quality) {
    std::vector<color_t> pixels;
    uint8_t* data_mask = (uint8_t*)mask_buffer.ptr;
    ...
    // for (int pixel_index=0; pixel_index < 80; pixel_index += quality) {
        // Alpha channel big enough
        
        // if (data[pixel_index * 4 + 3] >= 125) {
        if(data_mask[pixel_index] > 125){
...
std::vector<color_tp> get_palette_mask(py::array_t<uint8_t,  py::array::c_style> image,py::array_t<uint8_t,  py::array::c_style>  mask, int color_count, int quality) {
    // bool c[num_mask_id];
    // for(int i = 0 ; i < num_mask_id;i++)
    //     c[i]=false;
    // py::buffer_info mask_idx_buffer = mask_idx.request();
    // uint8_t* mask_idx_data = (uint8_t*)mask_idx_buffer.ptr;
    // for(int i = 0 ; i <mask_idx_buffer.shape[0];i++){
    //     c[mask_idx_data[i]]=true;
    ...
    if (mask.ndim != 2) throw std::runtime_error("Mask must be 2D matrix (height x width)");
    ...
    // for (int pixel_index=0; pixel_index < 80; pixel_index += quality) {
        ...
        if(data_mask[pixel_index] > 125){
            ...
            // std::cout<<int(data[pixel_index * 4 ])<<" "<<int(data[pixel_index * 4 +1])<<" "<<int(data[pixel_index * 4 +2])<<"\n";
            // Not white
            pixels.push_back({data[pixel_index * 3], data[pixel_index * 3 + 1], data[pixel_index * 3 + 2]});
...
std::vector<color_tp> get_palette(py::array_t<uint8_t,  py::array::c_style> image, int color_count, int quality) {
...
PYBIND11_MODULE(fast_colorthief_backend, m) {
    ...
    m.def("get_palette_mask", &get_palette_mask, "Return color palette from mask");
};
